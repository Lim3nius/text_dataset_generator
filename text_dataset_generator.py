...
from helpers import semantic_segmentation_helper as sem_seg_help
...
from helpers import generator as genm
...
def parse_arguments():
    ...
    pd.set_defaults(func=genm.generate)
...
def generator(config, content, index, fonts, backgrounds, args,
    ...
    while content and generated < to_generate:
        ...
        try:
            tmp = text_renderer.render_page(face, content, config)
            text_img, annotations, baselines, new_content = tmp
            ...
            log.warn(
                f'There was an error during creating image number {index}\n'
                f'Text: {content[0][:30]}\n'
                f'Font: {font_name}\n'
                'Trying to generate same text again.')
        ...
        annotations = update_annotations(
            annotations, config['Padding']['left'], config['Padding']['top'])
        baselines = update_baselines(
            baselines, config['Padding']['left'], config['Padding']['top'])

        semantic_segmentation_image = None
        image_name = prefix + "_" + str(index)

        if config['Common']['semanticsegmentation']:
            semantic_segmentation_image = sem_seg_help.generate(
                text_img, annotations)
        ...
        try:
            ...
            traceback.print_exc()
            log.warn(
                'There was an error during applying effects'
                f'on image number {index}\n'
                f'Text: {content[0][:30]}\n'
                f'Font: {font_name}\n'
                f'Trying to generate same text again.\n'
                f'Skipping 10 characters. Reason: Unable to generate paragraph'
            )
            content[0] = content[0][10:]
            continue
        ...
        transkribus = xml_helper.annotations_and_baselines_to_transkribus_xml(
            annotations, baselines, image_name + ".png", result.shape[:2])
        file_helper.write_file(transkribus, path + ".xml")

        file_helper.write_image(result, path + ".png")

        if config['Common']['semanticsegmentation'] and \
                semantic_segmentation_image is not None:
            file_helper.write_image(
                semantic_segmentation_image, path + "_semantic.png")
            manifest_row['semanticsegmentation'] = image_name + '_semantic.png'

        file_helper.write_annotation_file(annotations, baselines, path +
                                          ".txt")

        if config['Common']['annotations']:
            result = image_helper.draw_annotations(
                result, annotations, baselines)
            file_helper.write_image(result, path + "_annotations.png")
            manifest_row['semanticsegmentation'] = image_name + \
                '_annotations.png'
...
class Storage:
    '''Storage's purpose is to group data used further in computation'''
