...
from helpers.effects import surrounding_text_effect, back_page_text_effect, printing_imperfections_effect, blurry_text_effect, salt_and_pepper
...
def apply_effects(img, font, background, config):
    ...
    if config['SaltAndPepper']['probability'] > 0:
        salt_and_pepper.addSaltAndPepper(img, config['SaltAndPepper']['probability'])
